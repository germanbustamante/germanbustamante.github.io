...
export async function GET(context: APIContext) {
  const { title, pubDate, description, tags, language } =
  ...
  const markup = html`
    ...
    >
      <div tw="flex flex-col w-full h-4/5 p-10 justify-center">
        ...
        <div
          tw="flex text-6xl mb-4 w-full font-bold leading-snug tracking-tight text-transparent bg-teal-400"
          style="background-clip: text; -webkit-background-clip: text; background: linear-gradient(90deg, rgb(13, 148, 136), rgb(45, 212, 191));"
        >
          ${title}
        </div>
